...
class Feeder(Dataset):
    ...
    def __getitem__(self, index):
        ...
        head_data = np.expand_dims(head_data, axis=-1)  
        hand_data = np.expand_dims(hand_data, axis=0)  
        hand_data = np.expand_dims(hand_data, axis=-1)  
        leg_data = np.expand_dims(leg_data, axis=0)  
        leg_data = np.expand_dims(leg_data, axis=-1) 
        full_body_data = np.expand_dims(full_body_data, axis=0)  
        full_body_data = np.expand_dims(full_body_data, axis=-1)  
